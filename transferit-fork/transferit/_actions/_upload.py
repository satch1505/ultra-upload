...
def do_upload(
    ...
    *,
    ...
    def _pick_pool(sz: int, force_refresh: bool = False) -> tuple[str, str]:
        nonlocal pools
        if force_refresh or not pools:
        ...
        if not force_refresh:
            return _pick_pool(sz, force_refresh=True)
    ...
    for f in files:
        ...
        for attempt in range(1, max_file_retries + 1):
            try:
                host, uri = _pick_pool(fsize, force_refresh=(attempt > 1))
                ul_key = rand_a32(6)
                ...
                last_exc = e
                log.warning("Upload failed for %s (attempt %d/%d): %s", f.name, attempt, max_file_retries, e)
                if attempt == max_file_retries:
                    raise
                time.sleep(2 * attempt)
                idx = fileno_provider()

        if token is None or macs is None:
            raise MegaAPIError(f"upload failed for {f.name}: {last_exc}")
